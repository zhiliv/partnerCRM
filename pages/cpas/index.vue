        ...
        <Column field="id" class="w-[180px]">
          <template #header>
            <app-header-column
              ...
              title="Идентификатор"
              field="id"
              v-model:filter="filter.cpa.id"
              v-model:sort="sort.cpa.id"
            />
          </template>
        </Column>
        <Column field="name" class="w-[75%]">
          <template #header>
            <app-header-column
              ...
              title="Наименование"
              field="id"
              v-model:filter="filter.cpa.name"
              v-model:sort="sort.cpa.name"
            />
          </template>
        </Column>
        <Column class="w-[350px]">
          <template #header>
            <app-header-column
              ...
              title="Дата создания"
              field="created_date"
              v-model:filter="filter.cpa.created_date"
              v-model:sort="sort.cpa.created_date"
            />
          </template>
          <template #body="{ data }">
...
const selectItem: Ref<FieldsCPA | null> = ref(null) // Выделенный элемент
...
const isLoading: ComputedRef<boolean> = computed(() => useStoreCPA.isLoading) // Вычисление значения загрузки данных
...
/**
 ** Получение списка "Партнерские программы"
 * @async
 * @function updateList
 */
const updateList: () => Promise<void> = async () => {
  await nextTick() // Ожидание загрузки DOM
  await storeCPA.getList() // Получение списка
}
...
/**
 ** Добавление новой партнерской программы
...
const onCreate = async (): Promise<void> => {
  const options: DynamicDialogOptions = {
    props: {
      header: 'Создать новую партнерскую программу',
    ...
    onClose: async (args: any): Promise<void> => {
      await updateList()
      selectItem.value = storeCPA.list[0] // Выделение созданного элемента(находится первый в списке)
    },
    data: {
      type: 'create', // Тип модального окна
    },
  } // Параметры модального окна
  dialog.open(editModal, options) // Открытие модельного окна для добавления новой партнерской программы
}

/**
 ** Редактирование партнерской программы
 * @function onEdit
 */
const onEdit = async (): Promise<void> => {
  const options: DynamicDialogOptions = {
    props: {
      header: 'Редактирование партнерской программы',
      draggable: true,
      position: 'right',
      style: {
        height: '100%',
...
const onDelete = async (): Promise<void> => {
  if (selectItem.value) {
    confirm.require({
      message: `Удалить партнерскую программу "${selectItem.value.name}"?`,
      ...
      accept: async () => {
        if (selectItem.value) {
          storeCPA.record = selectItem.value
          const resDel: boolean = await storeCPA.del()
          if (resDel) {
            await updateList()
          }
        }
...
const setPaginaion = async () => {
  storeCPA.offset = valuePagination.value * storeCPA.limit
  await updateList()
}

watch(filter, async (newVal) => {
  storeCPA.filter = newVal // Установка фильтра
  await updateList()
  selectItem.value = null
})

watch(sort, async (newVal: any) => {
  storeCPA.sort = newVal // Установка сортировки
  await updateList()
  selectItem.value = null
})
</script>
