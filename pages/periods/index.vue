<template>
  ...
  <div class="pt-1 shadow-xl">
        ...
        <Column field="id" class="w-[180px]">
          <template #header>
            <app-header-column
              ...
              title="Идентификатор"
              field="id"
              v-model:filter="filter.prd.id"
              v-model:sort="sort.prd.id"
            />
          </template>
        </Column>
        <Column field="name" class="w-[75%]">
          <template #header>
            <app-header-column
              ...
              title="Наименование"
              field="id"
              v-model:filter="filter.prd.name"
              v-model:sort="sort.prd.name"
            />
          </template>
        </Column>
        <Column field="padez" class="w-[75%]">
          <template #header>
            <app-header-column
              type="string"
              title="Родительский падеж"
              field="id"
              v-model:filter="filter.prd.padez"
              v-model:sort="sort.prd.padez"
            />
          </template>
        </Column>
        <Column field="mnozh" class="w-[75%]">
          <template #header>
            <app-header-column
              type="string"
              title="Множественное число"
              field="id"
              v-model:filter="filter.prd.mnozh"
              v-model:sort="sort.prd.mnozh"
            />
          </template>
        </Column>
      </DataTable>
    ...
    <div ref="blockPagination">
      <Paginator
        :rows="storePeriods.limit"
        :totalRecords="storePeriods.count"
        ref="pagination"
        @click="setPaginaion"
      />
    </div>
...
import type { Period, FieldsPeriod } from '~/types/Period' 
...
const selectItem: Ref<FieldsPeriod | null> = ref(null) // Выделенный элемент
...
const isLoading: ComputedRef<boolean> = computed(() => storePeriods.isLoading) // Вычисление значения загрузки данных
...
const onCreate = async (): Promise<void> => {
  const options: DynamicDialogOptions = {
    ...
    onClose: async (args: any): Promise<void> => {
      await updateList()
      selectItem.value = storePeriods.list[0] // Выделение созданного элемента(находится первый в списке)
    },
    data: {
      type: 'create', // Тип модального окна
    },
  } // Параметры модального окна
  dialog.open(editModal, options) // Открытие модельного окна для добавления нового периода
}
...
const onDelete = async (): Promise<void> => {
  if (selectItem.value) {
    confirm.require({
      message: `Удалить период "${selectItem.value.name}"?`,
      ...
      accept: async () => {
        if (selectItem.value) {
          storePeriods.record = selectItem.value
          const resDel: boolean = await storePeriods.del()
          if (resDel) {
            await updateList()
          }
        }
...
const setPaginaion = async () => {
  storePeriods.offset = valuePagination.value * storePeriods.limit
  await updateList()
}

watch(filter, async (newVal) => {
  storePeriods.filter = newVal // Установка фильтра
  await updateList()
  selectItem.value = null
})

watch(sort, async (newVal: any) => {
  storePeriods.sort = newVal // Установка сортировки
  await updateList()
  selectItem.value = null
})
</script>
