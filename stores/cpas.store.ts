import type { CPA, FieldsCPA } from '~/types/CPA'
import type { ResponseHTTP } from '~/types/ResponseHTTP'
import { defineStore } from 'pinia'


export const useStoreCPA = defineStore('cpas', () => {
  const list = ref<FieldsCPA[]>([]) // Список строк полученных из таблицы
  ...
  const record = ref<FieldsCPA>({
    id: null,
    name: null,
    created_date: null
  }) // Запись
  const filter = ref<FieldsCPA | any>({})
  ...
  */
  const getList = async (): Promise<boolean> => {
    ...
    await getCount()
    const url: string = `/api/cpas/all` // Ссылка для получения данных
    try {
  ...
  */
  const getCount = async (): Promise<boolean> => {
    ...
    const url: string = `/api/cpas/count` // Ссылка для получения количества записей
    try {
  ...
  */
  const create = async (): Promise<boolean> => {
    const url: string = `/api/cpas/add` // Ссылка 
    try {
  ...
  */
  const get = async (): Promise<boolean> => {
    ...
    const url: string = `/api/cpas/get` // Ссылка
    try {
  ...
  */
  const update = async (): Promise<boolean> => {
    const url: string = `/api/cpas/update` // Ссылка
    try {
  ...
  */
  const del = async (): Promise<boolean> => {
    const url: string = `/api/cpas/del` // Ссылка
    try {
